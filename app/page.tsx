...
export default function StreamWiseHyperUltimate() {
  ...
  useEffect(() => { 
    setIsMounted(true);
    const saved = localStorage.getItem('sw_hyper_vault');
    if (saved) setWishlist(JSON.parse(saved));
    
    const handlePop = () => { if(selected) setSelected(null); };
    window.addEventListener('popstate', handlePop);
    return () => window.removeEventListener('popstate', handlePop);
  }, [selected]);

  useEffect(() => {
    if (isMounted) localStorage.setItem('sw_hyper_vault', JSON.stringify(wishlist));
  }, [wishlist, isMounted]);
  ...
  const syncSystem = useCallback(async () => {
    ...
    try {
      ...
      if (searchQuery.trim().length > 0) {
        endpoints = [{ key: `Search Intel: ${searchQuery}`, url: `/search/multi?api_key=${API_KEY}&query=${encodeURIComponent(searchQuery)}` }];
      } else {
        endpoints = [
          { key: `Trending Globally`, url: `/trending/all/week?api_key=${API_KEY}` },
          { key: `IMDB Legends`, url: `/movie/top_rated?api_key=${API_KEY}${langQ}` },
          { key: `${LANGUAGES.find(l => l.id === activeLang)?.name} Blockbusters`, url: `/discover/movie?api_key=${API_KEY}${langQ}${genreQ}&sort_by=popularity.desc` },
          { key: `Featured Releases`, url: `/movie/upcoming?api_key=${API_KEY}${langQ}` }
        ];
      }
      ...
      setSectors(resObj);
    } catch (e) { console.error("Archive Link Broken"); }
    ...
    <div className="bg-[#0A070B] text-white min-h-screen font-sans selection:bg-fuchsia-600/40 overflow-x-hidden">
      <nav className="fixed top-0 w-full z-[800] bg-[#0A070B]/95 backdrop-blur-3xl border-b border-white/5">
        <div className="flex items-center justify-between px-6 md:px-16 py-6">
            ...
            <div className="flex items-center gap-6">
                ...
                <input type="text" value={searchQuery} placeholder="SYSTEM SCAN..." onChange={(e) => {setSearchQuery(e.target.value); setShowWishlist(false); setViewAllSector(null);}} className="bg-white/5 border border-white/10 rounded-2xl px-6 py-3 text-[10px] font-black tracking-widest outline-none focus:border-fuchsia-600 w-40 md:w-96" />
            </div>
        </div>
        <div className="px-6 md:px-16 pb-6 overflow-x-auto no-scrollbar relative z-[900]">
          <div className="inline-flex items-center gap-4">
            <div className="flex gap-2">
                {LANGUAGES.map((l) => (
                  <button key={l.id} onClick={() => { setSearchQuery(""); setViewAllSector(null); setActiveLang(l.id); }} className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase border transition-all ${activeLang === l.id ? 'bg-white text-black' : 'border-white/10 text-zinc-500'}`}>{l.name}</button>
                ))}
            </div>
            <div className="w-[1px] h-4 bg-white/20" />
            <div className="flex gap-2">
                {GENRES.map((g) => (
                  <button key={g.id} onClick={() => { setSearchQuery(""); setViewAllSector(null); setActiveGenre(g.id); }} className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase border transition-all ${activeGenre === g.id ? 'bg-fuchsia-600 border-fuchsia-600 shadow-lg' : 'border-white/10 text-zinc-400'}`}>{g.name}</button>
                ))}
            </div>
          </div>
        </div>
      ...
      <main className="px-6 md:px-16 pt-52 pb-40">
        <div className="space-y-40">
          {Object.entries(showWishlist ? { "Saved Archive": wishlist } : sectors).map(([title, items]) => {
            const isExpanded = viewAllSector === title;
            return (
              <section key={title}>
                <div className="flex justify-between items-center mb-10">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.6em] text-white/30 flex items-center gap-4"><span className="w-10 h-[1px] bg-fuchsia-600" /> {title}</h3>
                  <button onClick={() => setViewAllSector(isExpanded ? null : title)} className="text-[9px] font-black uppercase tracking-[0.2em] text-fuchsia-600 hover:text-white transition-all border-b border-fuchsia-600/30 pb-1">{isExpanded ? "Collapse" : "Explore Sector +"}</button>
                </div>
                <div className={`${isExpanded ? 'grid grid-cols-2 md:grid-cols-5 gap-8' : 'flex gap-6 overflow-x-auto no-scrollbar pb-10'}`}>
                  {items?.map((m, idx) => (
                    <div key={`${m.id}-${idx}`} onClick={() => openIntel(m)} className={`${isExpanded ? 'w-full' : 'shrink-0 w-44 md:w-64'} group cursor-pointer relative`}>
                      <div className="aspect-[2/3] rounded-[2.5rem] overflow-hidden border border-white/5 relative z-10 transition-all group-hover:border-fuchsia-600 bg-zinc-900">
      ...
      {selected && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#0F0D12] w-full max-w-7xl h-[85vh] rounded-[3.5rem] border border-white/10 flex flex-col md:flex-row overflow-hidden shadow-3xl animate-in zoom-in-95 duration-300">
            <div className="md:w-3/5 relative p-12 flex flex-col justify-end overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="flex gap-4 mb-8">
                    ...
                    }} className={`px-10 py-4 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${wishlist.find(x => x.id === selected.id) ? 'bg-fuchsia-600 border-fuchsia-600' : 'bg-white/10 border border-white/10 hover:bg-white hover:text-black'}`}>{wishlist.find(x => x.id === selected.id) ? '✓ Saved' : '+ Save to Vault'}</button>
                    <button onClick={() => window.open(`https://www.imdb.com/find?q=${encodeURIComponent(selected.title || selected.name)}`, '_blank')} className="px-10 py-4 rounded-full bg-[#f3ce13] text-black text-[10px] font-black uppercase tracking-widest hover:bg-white">IMDB</button>
                </div>
                {/* CINEMA MODE: EMBEDDED TRAILER */}
                {trailerKey && (
                  <div className="w-full aspect-video rounded-3xl overflow-hidden border border-white/10 mb-8 bg-black">
                    <iframe src={`https://www.youtube.com/embed/${trailerKey}?autoplay=0&mute=0`} className="w-full h-full" allowFullScreen />
                  </div>
                )}
                <p className="text-zinc-500 text-sm md:text-lg italic max-w-xl line-clamp-2 leading-relaxed">"{selected.overview}"</p>
              </div>
            </div>

            <div className="md:w-2/5 p-12 overflow-y-auto no-scrollbar border-l border-white/5 bg-white/[0.01]">
                <p className="text-fuchsia-500 text-[10px] font-black uppercase tracking-[0.4em] mb-8">Provider Nodes & Amounts</p>
                <div className="grid gap-3 mb-12">
                    {providers.map((p, i) => (
                        <div key={i} className="flex items-center justify-between bg-white/5 p-5 rounded-[2rem] border border-white/10 hover:bg-white/10 transition-all">
                            ...
                            <span className="text-[9px] font-black text-fuchsia-500 bg-fuchsia-500/10 px-3 py-1 rounded-full border border-fuchsia-500/20 uppercase">${Math.floor(selected.popularity || 0)} VAL</span>
                        </div>
                    ))}
                </div>
                {/* AI RECOMMENDATIONS SECTION */}
                <p className="text-fuchsia-600 text-[10px] font-black uppercase tracking-[0.4em] mb-6">AI Discovery Node</p>
                <div className="grid grid-cols-2 gap-4 mb-12">
                   {recommendations.map((r, i) => (
                     <div key={i} onClick={() => openIntel(r)} className="cursor-pointer group">
                       <div className="aspect-[2/3] rounded-2xl overflow-hidden border border-white/5 group-hover:border-fuchsia-600 transition-all">
                         <img src={`${THUMB_BASE}${r.poster_path}`} className="w-full h-full object-cover" alt="r" />
                       </div>
                     </div>
                   ))}
                </div>
                <p className="text-white/20 text-[10px] font-black uppercase tracking-[0.4em] mb-8">Personnel</p>
