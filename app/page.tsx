...
export default function StreamWiseEmpire() {
  ...
  const [minRating, setMinRating] = useState(0); // DEEP FILTER
  const [releaseYear, setReleaseYear] = useState(""); // DEEP FILTER
  const [isMounted, setIsMounted] = useState(false);

  useEffect(() => { 
    setIsMounted(true);
    const saved = localStorage.getItem('sw_empire_vault');
    if (saved) setWishlist(JSON.parse(saved));
  }, []);

  useEffect(() => {
    if (isMounted) localStorage.setItem('sw_empire_vault', JSON.stringify(wishlist));
  }, [wishlist, isMounted]);
  ...
  const syncSystem = useCallback(async () => {
    ...
    try {
      ...
      if (searchQuery.trim().length > 0) {
        endpoints = [{ key: `Results: ${searchQuery}`, url: `/search/multi?api_key=${API_KEY}&query=${encodeURIComponent(searchQuery)}` }];
      } else {
        endpoints = [
          { key: `Trending Now`, url: `/trending/all/week?api_key=${API_KEY}${voteQ}` },
          { key: `Premium Picks`, url: `/discover/movie?api_key=${API_KEY}${langQ}${genreQ}${yearQ}${voteQ}&sort_by=popularity.desc` },
          { key: `IMDB Legends`, url: `/movie/top_rated?api_key=${API_KEY}${langQ}${voteQ}` },
          { key: `New Arrivals`, url: `/movie/upcoming?api_key=${API_KEY}${langQ}${yearQ}` }
        ];
      }
  ...
  return (
    <div className="bg-[#0A070B] text-white min-h-screen font-sans selection:bg-fuchsia-600/40 overflow-x-hidden">
      ...
      {/* PREMIUM NAVIGATION WITH DEEP FILTERS */}
      <nav className="fixed top-0 w-full z-[800] bg-[#0A070B]/90 backdrop-blur-3xl border-b border-white/5">
        <div className="flex flex-col md:flex-row items-center justify-between px-6 md:px-16 py-6 gap-4">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
                <input type="text" value={searchQuery} placeholder="SCAN DATABASE..." onChange={(e) => {setSearchQuery(e.target.value); setShowWishlist(false);}} className="bg-white/5 border border-white/10 rounded-2xl px-6 py-3 text-[10px] font-black tracking-widest outline-none focus:border-fuchsia-600 w-64 lg:w-96" />
                <input type="number" placeholder="YEAR" value={releaseYear} onChange={(e) => setReleaseYear(e.target.value)} className="bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-[10px] font-black w-20 outline-none focus:border-fuchsia-600" />
                <select onChange={(e) => setMinRating(Number(e.target.value))} className="bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-[10px] font-black outline-none focus:border-fuchsia-600 text-white cursor-pointer">
                    <option value="0" className="bg-[#0A070B]">ALL RATINGS</option>
                    <option value="7" className="bg-[#0A070B]">7.0+ ★</option>
                    <option value="8" className="bg-[#0A070B]">8.0+ ★</option>
                </select>
                <button onClick={() => setShowWishlist(!showWishlist)} className={`px-6 py-3 rounded-2xl border text-[10px] font-black uppercase transition-all ${showWishlist ? 'bg-fuchsia-600 border-fuchsia-600' : 'border-white/10 text-white/40'}`}>Vault ({wishlist.length})</button>
            </div>
        </div>
      ...
      <main className="px-6 md:px-16 pt-64 pb-40">
        <div className="space-y-44">
          {Object.entries(showWishlist ? { "Saved Records": wishlist } : sectors).map(([title, items]) => {
            ...
            return (
              <section key={title}>
                <div className="flex justify-between items-center mb-10">
                  ...
                  <button onClick={() => setViewAllSector(isExpanded ? null : title)} className="text-[9px] font-black text-fuchsia-600 uppercase underline underline-offset-8 decoration-fuchsia-600/30">{isExpanded ? "Collapse" : "Explore Sector +"}</button>
                </div>
                  ...
                  {items?.map((m, idx) => (
                    <div key={`${m.id}-${idx}`} onClick={() => openIntel(m)} className={`${isExpanded ? 'w-full' : 'shrink-0 w-48 md:w-72'} group cursor-pointer relative`}>
                      <div className="aspect-[2/3] rounded-[3rem] overflow-hidden border border-white/5 relative z-10 transition-all group-hover:border-fuchsia-600 bg-zinc-900 shadow-2xl">
                        ...
                        <div className="absolute top-6 right-6 z-20 bg-black/60 backdrop-blur-xl px-3 py-1.5 rounded-xl border border-white/10 opacity-0 group-hover:opacity-100 transition-all">
      ...
      {selected && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center">
          <div className="absolute inset-0 bg-black/98 backdrop-blur-md" onClick={() => setSelected(null)} />
          
          <div className="relative bg-[#0F0D12] w-full h-full md:h-[92vh] md:w-[96vw] md:rounded-[4rem] border border-white/10 overflow-hidden shadow-3xl flex flex-col md:flex-row animate-in slide-in-from-bottom-10 duration-500">
            
            <div className="absolute inset-0 z-0 pointer-events-none">
               {trailerKey ? (
                 <iframe src={`https://www.youtube.com/embed/${trailerKey}?autoplay=1&mute=1&controls=0&loop=1&playlist=${trailerKey}&modestbranding=1`} className="w-[200%] h-[200%] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-30" />
               ) : (
                 <img src={`${IMAGE_BASE}${selected.backdrop_path}`} className="w-full h-full object-cover opacity-20" alt="bg" />
               )}
               <div className="absolute inset-0 bg-gradient-to-t from-[#0F0D12] via-[#0F0D12]/60 to-transparent" />
            </div>

            <div className="relative z-10 w-full flex flex-col md:flex-row overflow-y-auto no-scrollbar">
                <div className="flex-1 p-8 md:p-24 flex flex-col justify-end">
                    <button onClick={() => setSelected(null)} className="absolute top-12 left-12 text-[10px] font-black uppercase text-white/40 hover:text-white transition-all">← Exit Dossier</button>
                    <h2 className="text-6xl md:text-[11rem] font-black uppercase tracking-tighter leading-[0.75] mb-8 italic">{selected.title || selected.name}</h2>
                    ...
                    <div className="flex flex-wrap gap-4 mb-12">
                        <button onClick={() => {
                            ...
                            {wishlist.find(x => x.id === selected.id) ? 'In Vault' : '+ Archive'}
                        </button>
                    ...
                    <p className="text-xl md:text-2xl text-white/60 max-w-3xl leading-relaxed italic mb-12">"{selected.overview}"</p>
                    
                    {/* ACTOR FILMOGRAPHY FEATURE: CLICKING SEARCHES BY ACTOR */}
                    <div className="flex flex-wrap gap-2">
                        {cast.map((c, i) => (
                          <button key={i} onClick={() => { setSearchQuery(c.name); setSelected(null); }} className="text-[10px] font-black uppercase text-white/40 bg-white/5 px-4 py-2 rounded-lg border border-white/5 hover:border-fuchsia-600 hover:text-fuchsia-500 transition-all">{c.name}</button>
                        ))}
                    </div>
                </div>

                <div className="w-full md:w-[500px] bg-black/60 backdrop-blur-3xl border-l border-white/10 p-12 overflow-y-auto no-scrollbar">
                    <p className="text-fuchsia-500 text-[10px] font-black uppercase tracking-[0.4em] mb-10">Active Streaming Nodes</p>
                    <div className="space-y-4 mb-20">
                        {providers.map((p, i) => (
                            <div key={i} className="flex items-center justify-between bg-white/5 p-6 rounded-3xl border border-white/5">
                    ...
                    <p className="text-white/30 text-[10px] font-black uppercase tracking-[0.4em] mb-8">AI Discovery Engine</p>
