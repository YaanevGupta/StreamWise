...
export default function StreamWiseIroncladV5() {
  ...
  const [viewAllSector, setViewAllSector] = useState<string | null>(null); // EXPLORE MORE LOGIC
  const [isMounted, setIsMounted] = useState(false);

  useEffect(() => { 
    setIsMounted(true);
    const saved = localStorage.getItem('sw_ironclad_vault');
    if (saved) setWishlist(JSON.parse(saved));
  }, []);

  useEffect(() => {
    if (isMounted) localStorage.setItem('sw_ironclad_vault', JSON.stringify(wishlist));
  }, [wishlist, isMounted]);
      ...
      <nav className="fixed top-0 w-full z-[800] bg-[#0A070B]/95 backdrop-blur-3xl border-b border-white/5">
        <div className="flex items-center justify-between px-6 md:px-16 py-6">
            ...
            <div className="flex items-center gap-6">
                ...
                <input 
                    type="text" value={searchQuery} placeholder="SYSTEM SEARCH..." 
                    onChange={(e) => {setSearchQuery(e.target.value); setShowWishlist(false); setViewAllSector(null);}}
        ...
        <div className="px-6 md:px-16 pb-6 overflow-x-auto no-scrollbar relative z-[900]">
          <div className="inline-flex items-center gap-4">
            <div className="flex gap-2">
                {LANGUAGES.map((l) => (
                  <button key={l.id} onClick={() => { setSearchQuery(""); setViewAllSector(null); setShowWishlist(false); setActiveLang(l.id); }} className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase border transition-all ${activeLang === l.id ? 'bg-white text-black' : 'border-white/10 text-zinc-500'}`}>{l.name}</button>
                ))}
            </div>
            <div className="w-[1px] h-4 bg-white/20" />
            <div className="flex gap-2">
                {GENRES.map((g) => (
                  <button key={g.id} onClick={() => { setSearchQuery(""); setViewAllSector(null); setShowWishlist(false); setActiveGenre(g.id); }} className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase border transition-all ${activeGenre === g.id ? 'bg-fuchsia-600 border-fuchsia-600' : 'border-white/10 text-zinc-400'}`}>{g.name}</button>
                ))}
            </div>
          </div>
        </div>
            ...
            return (
              <section key={title} className="relative">
                <div className="flex justify-between items-center mb-10">
                  ...
                  {/* EXPLORE MORE BUTTON - WORKING */}
                  <button 
                    onClick={() => setViewAllSector(isExpanded ? null : title)}
                    className="text-[9px] font-black uppercase tracking-[0.2em] text-fuchsia-600 hover:text-white transition-all border-b border-fuchsia-600/30 pb-1"
                  >
                    {isExpanded ? "Collapse Section" : "Explore Sector +"}
                  </button>
                </div>
            ...
            <div className="md:w-3/5 relative p-12 flex flex-col justify-end">
              ...
              <div className="relative z-10">
                ...
                <div className="flex gap-4 mb-8">
                    ...
                    <button onClick={() => window.open(`https://www.imdb.com/find?q=${encodeURIComponent(selected.title || selected.name)}`, '_blank')} className="px-10 py-4 rounded-full bg-[#f3ce13] text-black text-[10px] font-black uppercase tracking-widest hover:bg-white">IMDB Archive</button>
                </div>
                <p className="text-zinc-500 text-sm md:text-lg italic max-w-xl line-clamp-3 leading-relaxed">"{selected.overview || "Deep records loading..."}"</p>
              </div>
            </div>
                ...
                <div className="grid gap-3">
                    {providers.map((p, i) => (
                        <div key={i} className="flex items-center justify-between bg-white/5 p-5 rounded-[2rem] border border-white/10 hover:bg-white/10 transition-all">
                            ...
                            <span className="text-[9px] font-black text-fuchsia-500 bg-fuchsia-500/10 px-3 py-1 rounded-full border border-fuchsia-500/20 uppercase">
                                ${Math.floor(selected.popularity || 0)} VAL
                            </span>
                        </div>
                    ))}
