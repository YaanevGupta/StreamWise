...
export default function StreamWiseAbsoluteFinal() {
  ...
  useEffect(() => { 
    setIsMounted(true);
    const saved = localStorage.getItem('sw_master_vault_vfinal');
    if (saved) {
  ...
  useEffect(() => {
    if (isMounted) localStorage.setItem('sw_master_vault_vfinal', JSON.stringify(wishlist));
  }, [wishlist, isMounted]);

  // 2. REAL-TIME AUTO-REFRESH ENGINE (The "Everything" Sync)
  const syncSystem = useCallback(async () => {
    ...
    try {
      ...
      // CRITICAL: Cache-bust ensures new movies appear instantly
      const liveSync = `&t=${new Date().getTime()}`;

      if (searchQuery.trim().length > 0) {
        // GLOBAL SEARCH: Accesses millions of titles from TMDB/IMDB archives
        endpoints = [{ key: `Global Intel: ${searchQuery}`, url: `/search/multi?api_key=${API_KEY}&query=${encodeURIComponent(searchQuery)}&include_adult=false${liveSync}` }];
      } else {
        ...
        endpoints = [
          ...
          { key: `Newly Released Series`, url: `/discover/tv?api_key=${API_KEY}${langQ}${genreQ}&sort_by=first_air_date.desc${liveSync}` }
        ];
      }
  ...
  // 3. HEARTBEAT: Auto-refresh data every 30 minutes without user interaction
  useEffect(() => {
    syncSystem();
    const interval = setInterval(syncSystem, 1800000); 
        ...
        <div className="flex items-center justify-between px-6 md:px-16 py-6">
            ...
            <div className="flex items-center gap-6">
                <button onClick={() => setShowWishlist(!showWishlist)} className={`text-[10px] font-black uppercase tracking-widest transition-all ${showWishlist ? 'text-fuchsia-500' : 'text-white/40'}`}>
                    The Vault ({wishlist.length})
                </button>
                <input 
                    type="text" value={searchQuery} placeholder="SEARCH GLOBAL ARCHIVES..." 
        ...
        <div className="px-6 md:px-16 pb-6 overflow-x-auto no-scrollbar scroll-smooth">
          <div className="inline-flex items-center gap-4 min-w-full">
            <div className="flex gap-2">
                {LANGUAGES.map((l) => (
                  <button key={l.id} onClick={() => {setActiveLang(l.id); setSearchQuery("");}} className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase border transition-all whitespace-nowrap ${activeLang === l.id ? 'bg-white text-black border-white' : 'border-white/10 text-zinc-500 hover:text-white'}`}>{l.name}</button>
                ))}
            </div>
            <div className="w-[1px] h-4 bg-white/20 shrink-0 mx-2" />
            <div className="flex gap-2">
                {GENRES.map((g) => (
                  <button key={g.id} onClick={() => {setActiveGenre(g.id); setSearchQuery("");}} className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase border transition-all whitespace-nowrap ${activeGenre === g.id ? 'bg-fuchsia-600 border-fuchsia-600' : 'border-white/10 text-zinc-400 hover:text-white'}`}>{g.name}</button>
                ))}
            </div>
          </div>
        </div>
      ...
      <main className="px-6 md:px-16 pt-52 pb-40">
        {showWishlist ? (
          <section className="animate-in fade-in duration-500">
            <h3 className="text-[11px] font-black uppercase tracking-[0.6em] text-fuchsia-500 mb-14">Personalized Vault</h3>
            <div className="grid grid-cols-2 md:grid-cols-6 gap-6">
                {wishlist.map(m => (
                    <div key={m.id} onClick={() => openIntel(m)} className="relative aspect-[2/3] rounded-[2.5rem] overflow-hidden border border-white/10 cursor-pointer shadow-2xl transition-transform hover:scale-95">
              ...
              <section key={title}>
                <div className="flex justify-between items-center mb-10">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.6em] text-white/30 flex items-center gap-4">
                     <span className="w-8 h-[1px] bg-fuchsia-600" /> {title}
                  </h3>
                  <button onClick={() => {setSearchQuery(title.split(' ')[0]); window.scrollTo(0,0);}} className="text-[9px] font-black uppercase tracking-widest text-fuchsia-600 hover:text-white transition-all underline underline-offset-8 decoration-fuchsia-600/30">Explore Sector +</button>
                </div>
                <div className="flex gap-6 overflow-x-auto no-scrollbar pb-10 scroll-smooth">
                  {items.map((m) => (
                    <div key={m.id} onClick={() => openIntel(m)} className="shrink-0 w-44 md:w-64 group cursor-pointer transition-all">
                      <div className="aspect-[2/3] rounded-[2.5rem] overflow-hidden border border-white/5 transition-all group-hover:border-fuchsia-600 shadow-2xl relative bg-zinc-900">
            ...
            <div className="md:w-3/5 relative p-12 flex flex-col justify-end">
              ...
              <div className="relative z-10">
                ...
                <div className="flex gap-4 mb-8">
                    <button onClick={() => toggleWishlist(selected)} className={`px-10 py-4 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${wishlist.find(x => x.id === selected.id) ? 'bg-fuchsia-600 border-fuchsia-600' : 'bg-white/10 border border-white/10 hover:bg-white hover:text-black'}`}>
                        {wishlist.find(x => x.id === selected.id) ? '✓ Saved to Vault' : '+ Save to Vault'}
                    </button>
                    <button onClick={() => window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(selected.title || selected.name)}+trailer`)} className="px-10 py-4 rounded-full bg-white text-black text-[10px] font-black uppercase tracking-widest hover:bg-fuchsia-600 hover:text-white transition-all">Intel Trailer</button>
                </div>
                <p className="text-zinc-500 text-sm md:text-lg italic max-w-xl line-clamp-3 leading-relaxed">"{selected.overview || "Database briefing unavailable for this record."}"</p>
              </div>
            </div>
            <div className="md:w-2/5 p-12 overflow-y-auto no-scrollbar border-l border-white/5 bg-white/[0.01]">
                <p className="text-fuchsia-500 text-[10px] font-black uppercase tracking-[0.4em] mb-8">Access Nodes</p>
                <div className="grid gap-3">
                    {providers.map(p => (
                        <div key={p.provider_id} className="flex items-center justify-between bg-white/5 p-5 rounded-[2rem] border border-white/10 hover:bg-white/10 transition-all">
                            ...
                            <span className="text-[8px] font-black opacity-30">Live Node</span>
                        </div>
                    ))}
                    {providers.length === 0 && <p className="text-[9px] text-white/20 uppercase font-black">No direct network nodes found. Use YouTube for preview.</p>}
                </div>
                <p className="text-white/20 text-[10px] font-black uppercase tracking-[0.4em] mt-12 mb-8">Production Personnel</p>
